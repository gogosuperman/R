...
#ibrary和require都可以载入包，但二者存在区别。
#在一个函数中，如果一个包不存在，执行到library将会停止执行，require则会继续执行。
data("ROCR.simple")
df <- data.frame(ROCR.simple)
df
df$predclass <- ifelse(df$predictions>0.5,1,0)
#用0.5當作門檻值 
...
library(knitr)#為了用kable
# 安裝 magrittr 套件 ---------
install.packages("magrittr")
# 載入 magrittr 套件 ---------
library(magrittr)
#kable(as.data.frame(ROCR.simple)[1:10, ]) %>% kable_styling() %>% column_spec(1:2,width = "20em")
#為了方便只看前10筆資料，並轉換型態
a = kable(as.data.frame(ROCR.simple)[1:10, ]) 
a
pred <- ROCR::prediction(ROCR.simple$predictions,ROCR.simple$labels)#要predict 的column
#ggplot2::ggplot()-->明確指出使用套件 ggplot2 內的函式 ggplot()
perf<- ROCR::performance(pred,"tpr","fpr")
#performance()函数通過真陰性tpr和假陰性fpr来計算roc曲線下的面積auc
perf
